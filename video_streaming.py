...
class VideoChatClient:
    ...
    def load_file(self):
        ...
        # -------------------------------
            ...
            comp_window.title("")
            tk.Label(comp_window, text='압축 방식을 고르시오').pack(anchor='center')

            button_frame = tk.Frame(comp_window)
            button_frame.pack(side=tk.TOP, pady=20, padx=30)

            zip_button = tk.Button(comp_window, text="ZIP", command=lambda: self._send_zip_video(path, comp_window))
            zip_button.pack(side=tk.LEFT, padx=15)

            h263_button = tk.Button(comp_window, text="H.263", command=lambda: self._send_h263_video(path, comp_window))
    ...
    def _send_zip_video(self, video_path: str, window):
        # zip 압축 import (extra.py)
        from extra import zip_compress_file

        # ZIP 압축 생성
        zip_path = zip_compress_file(video_path)
    ...
    def _send_h263_video(self, video_path: str, window):
        compressed_path = video_path + ".avi"
        encode_h263(video_path, compressed_path)

        size = os.path.getsize(compressed_path)

        meta = {"filename": os.path.basename(compressed_path), "filesize": size}
        self.send_bytes(TYPE_FILE_HDR, json.dumps(meta).encode("utf-8"))
        ...
        self.append_chat_system(
            f"동영상 H.263 압축 후 전송 완료: {os.path.basename(compressed_path)} ({size} bytes)"
        )
        window.destroy()
